...
export class ConsoleMailService implements IMailService {
  ...
  async sendMail(to: string, subject: string, template: string, context: any): Promise<void> {
    ...
    if (!process.env.BREVO_SMTP_KEY) {
      this.logger.warn(`BREVO_SMTP_KEY no configurada. Simulando envío a ${to}: ${subject}`);
      return;
    }
    ...
    try {
      await this.transporter.sendMail({
        ...
        to,
        subject,
        html: this.buildHtml(template, context),
      });
  ...
  private buildHtml(template: string, context: any): string {
    const estilos = `font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f9f9f9;`;
    const estilosBox = `background: white; padding: 30px; border-radius: 8px; border-left: 4px solid #0A2540;`;

    switch (template) {
      case 'cambio-estado-evento':
        return `<div style="${estilos}"><div style="${estilosBox}">
